...
export const FloatingForm = (props: {
  apiEndpoint: string;
    ...
    const { register, handleSubmit, formState: { errors } } = useForm<DownloadUrlModel>({
          ...
          <CardContent className="p-3">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
              <div className="space-y-2">
                <Label htmlFor="file" className="text-xs font-medium">File Upload</Label>
                <Input 
                  id="file" 
                  type="file" 
                  {...register('file')} 
              ...
              <div className="space-y-2">
                ...
                <Input id="filename" {...register('filename')} className="h-8 text-xs" />
                {errors.filename && <p className="text-destructive text-xs">{errors.filename.message}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="custom_path" className="text-xs font-medium">Path</Label>
                <Input id="custom_path" {...register('custom_path')} className="h-8 text-xs" />
                {errors.custom_path && <p className="text-destructive text-xs">{errors.custom_path.message}</p>}
              </div>
              <Button type="submit" className="w-full h-8 text-xs">Submit</Button>
            </form>
          </CardContent>
        )}
      </Card>
