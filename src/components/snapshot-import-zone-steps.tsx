...
interface SnapshotImportZoneStepsProps {
  children: ReactNode;
  validation: MachineStepValidation;
  setValidation: (newValidation: MachineStepValidation) => void;
  className?: string;
  showImportButton?: boolean;
}

export function SnapshotImportZoneSteps({
  children,
  validation,
  setValidation,
  className = "",
  showImportButton = true,
}: SnapshotImportZoneStepsProps) {
  ...
  const handleFileImport = (file: File) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!content.git_custom_nodes) {
          toast.error("No custom nodes found in snapshot");
          return;
        }
  ...
  const handleUpdateExisting = () => {
    ...
    let updatedCount = 0;
    const updated = validation.docker_command_steps.steps.map(
      (step: DockerCommandStep) => {
        if (step.type === "custom-node") {
          const url = (step.data as CustomNodeData).url;
          ...
          if (matchingNode) {
            ...
            if (hash !== (step.data as CustomNodeData).hash) {
              updatedCount++;
            }
            return {
              ...step,
    ...
    setValidation({
      ...validation,
      docker_command_steps: { steps: updated as DockerCommandStep[] },
      ...(snapshotContent?.comfyui && {
        comfyUiHash: snapshotContent.comfyui,
        selectedComfyOption: "custom",
      }),
    });
    setShowDialog(false);
    toast.success(`Updated ComfyUI hash to ${snapshotContent.comfyui}`);
    toast.success(
      `Updated ${updatedCount} custom node${updatedCount !== 1 ? "s" : ""}`,
    );
  };
  ...
  const handleImportAll = () => {
    ...
    const newSteps = Object.entries(snapshotContent.git_custom_nodes).map(
      ([url, data]: [string, any]) => ({
        ...
        data: {
          ...
          meta: {
            message: "Imported from snapshot",
            committer: {
              name: url.split("/")[3],
              date: new Date().toISOString(),
            },
    ...
    setValidation({
      ...validation,
      docker_command_steps: { steps: newSteps as DockerCommandStep[] },
      ...(snapshotContent?.comfyui && {
        comfyUiHash: snapshotContent.comfyui,
        selectedComfyOption: "custom",
      }),
    ...
    setShowDialog(false);
    toast.success(`Updated ComfyUI hash to ${snapshotContent.comfyui}`);
    toast.success(`Imported ${newSteps.length} custom nodes`);
  };
      ...
      {showImportButton && (
        <div className="flex justify-between pb-3">
          <span className="text-gray-500 text-sm">
            Drag and drop snapshot.json file here to import custom nodes.
          </span>
          <Button
            variant="outline"
