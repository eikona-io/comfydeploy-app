  ...
  return (
    <div className="my-12 h-4 flex flex-row bg-gray-200 dark:bg-gray-800 relative">
      <RunTimelineItem
        ...
        final={end}
        label="Queue"
        className="bg-[repeating-linear-gradient(45deg,theme(colors.gray.300)_0px,theme(colors.gray.300)_6px,theme(colors.gray.200)_6px,theme(colors.gray.200)_12px)]"
      />
      <RunTimelineItem
        start={start}
        since={queued}
      ...
      {end && (
        ...
        >
          {formatTime((end.getTime() - (start?.getTime() || 0)) / 1000)}
        </div>
      )}
      <motion.div className="absolute left-0 text-xs text-muted-foreground h-[20px] -top-[2px] w-[2px] rounded bg-gray-600 dark:bg-gray-200 drop-shadow-md" />
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    const callback = () => {
      ...
      const width = Math.max(
        ...
        Math.max(
          0,
          (until ? until.getTime() : now.getTime()) - since!.getTime(),
        ) / total,
      );
      ...
      if (tooltipRef.current) {
        tooltipRef.current.textContent = formatTime(
          ((until ? until.getTime() : new Date().getTime()) -
            since!.getTime()) /
            1000,
        );
      }
  ...
  return (
    <Tooltip>
      ...
      <TooltipTrigger
        ...
        ref={timerRef}
        className={cn(
          "transition-all shadow-sm duration-200 h-4 relative overflow-visible",
          className,
          since ? "" : "bg-gray-200",
        )}
      >
          ...
          <AnimatePresence>
            {start && !isLast && (
              <LiveTime
                ...
                until={until}
                className={cn(
                  "transition-all duration-200 absolute -top-5 right-0 w-fit whitespace-nowrap z-10 text-[11px] transform -translate-x-1/2",
                  evade ? "-top-10" : "",
                )}
              />
            )}
            ...
            {!hideLabel && (
              <motion.div className="absolute right-0 text-xs text-muted-foreground h-[20px] -top-[2px] w-[2px] rounded bg-gray-600 dark:bg-gray-200 drop-shadow-md" />
            )}

            {since && label && !hideLabel && (
              <motion.div
