...
export function SDInputsRender({
  ...
  try {
    ...
    const inputInfo = `"${inputNode.input_id}"${inputNode.display_name ? ` (${inputNode.display_name})` : ""}`;
    const typeInfo = `${inputNode.class_type}`;
    const errorMsg = error instanceof Error ? error.message : "Unknown error";

    throw new Error(`${inputInfo} | ${typeInfo} | ${errorMsg}`);
  }
}
